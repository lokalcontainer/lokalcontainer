...
export default class VariableTools implements IFontInfo {
    ...
    getAxes() {
        const fvar = this.getFvarTable();
        if (fvar) return fvar.axes;
        return null;
    }
    ...
    getAxesCount() {
        const fvar = this.getFvarTable();
        if (fvar) {
            if (fvar.axes && fvar.axes.length > 0) return fvar.axes.length;
        }
        return 0;
    }
    ...
    getAxis(i: number) {
        const fvar = this.getFvarTable();
        if (fvar) return fvar.axes[i];
        return null;
    }

    getAxisName(i: number) {
        const fvar = this.getFvarTable();
        if (fvar) return fvar.axes[i].name.en;
        return null;
    }

    getInstances() {
        const fvar = this.getFvarTable();
        if (fvar) return fvar.instances;
        return null;
    }
    ...
    getInstanceName(i: number) {
        const fvar = this.getFvarTable();
        if (fvar) return fvar.instances[i].name.en;
        return null;
    }

    getNamedInstance(i: number) {
        const fvar = this.getFvarTable();
        if (fvar) return fvar.instances[i];
        return null;
    }
